document.addEventListener('DOMContentLoaded', () => {
    ...
    // Включение / Выключение режима множественного выбора
    ...
    if (bulkDeleteBtn) {
        bulkDeleteBtn.addEventListener('click', async () => {
            ...
            try {
                const res = await fetch('/techpanel/api/users/delete_bulk', {
                    method: 'POST',
    ...
    // НАЖАТИЕ НА ШЕСТЕРЕНКУ (Открытие редактирования конкретного юзера)
    ...
    if (editForm) {
        editForm.addEventListener('submit', async (e) => {
            ...
            try {
                const res = await fetch('/techpanel/api/users/update', {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
                    body: JSON.stringify({ 
    ...
    if (deleteUserBtn) {
        deleteUserBtn.addEventListener('click', async () => {
            ...
            try {
                const res = await fetch('/techpanel/api/users/delete', {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
